...
class FavoriteManagement(private val system: SystemNet) {
    ...
    fun isFavorite(user: ShortUserGithub): Boolean {
        fillFavorites()
    ...
    fun sortFavorites(users: ArrayList<ShortUserGithub>): ArrayList<UserModel> {
        val ret = ArrayList<UserModel>(users.size)
        for (user in users) ret.add(UserModel(user, isFavorite(user)))
        return ret
    }

    fun addToFavorites(user: ShortUserGithub) {
        fillFavorites()
        favorites!!.add(0, UserModel(user, true))
        val fav =
            Array<String>(favorites!!.size, { i -> favorites!![i].id.toString() + divider + favorites!![i].login })
        system.saveStringArray(FAVORITES, fav)
    }

    fun removeFromFavorites(user: ShortUserGithub) {
        fillFavorites()
