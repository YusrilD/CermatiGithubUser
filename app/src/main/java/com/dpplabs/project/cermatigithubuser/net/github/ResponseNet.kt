...
object ResponseNet {
    ...
    fun parseSearchResults(response: String): Pair<ArrayList<ShortUserGithub>, Int> {
        try {
            val parser = JsonParser()
            val mainObject = parser.parse(response).asJsonObject

            val totalCount = mainObject.get("total_count").asInt
            if (totalCount == 0) Pair(ArrayList<ShortUserGithub>(), 0)

            val users = mainObject.getAsJsonArray("items")
            val userShorts = ArrayList<ShortUserGithub>()
    ...
    @Throws(ParseException::class)
    private fun initUserShort(user: JsonObject): ShortUserGithub {
        try {
            val id = user.get("id").asInt
            val login = user.get("login").asString
            return ShortUserGithub(id, login)
        } catch (t: Throwable) {
            throw ParseException()
        }
    ...
    @Throws(ParseException::class)
    fun parseRepositories(response: String): ArrayList<RepositoryGithub> {
        try {
            val parser = JsonParser()
            val result = ArrayList<RepositoryGithub>()
    ...
    @Throws(ParseException::class)
    fun parseUser(response: String): UserGithub {
        try {
            val parser = JsonParser()
            val mainObject = parser.parse(response).asJsonObject
